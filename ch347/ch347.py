...
class SPIConfig(ctypes.Structure):
    _fields_ = [
        ...
        ("ChipSelect", ctypes.c_ulong),                 # Chip select control. Bit 7 as 0 ignores chip select control,
        ...
        ("DelayDeactive", ctypes.c_ulong)               # Delay time for executing read/write operations after chip select is de-asserted, in microseconds
    ]
...
class CH347:
    ...
    def __init__(self, device_index=0, dll_path="lib/CH347DLLA64.DLL"):
        ...
        self.ch347dll.CH347GetDeviceInfor.argtypes = [ctypes.c_ulong, ctypes.POINTER(DeviceInfo)]
        ...
        self.ch347dll.CH347GetVersion.argtypes = [ctypes.c_ulong, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte)]
        ...
        self.ch347dll.CH347SetDeviceNotify.argtypes = [ctypes.c_ulong, ctypes.c_char_p, ctypes.CFUNCTYPE(None, ctypes.c_ulong)]
        ...
        self.ch347dll.CH347SPI_SetChipSelect.argtypes = [ctypes.c_ulong, ctypes.c_ushort, ctypes.c_ushort, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong]
        ...
        self.ch347dll.CH347SPI_Write.argtypes = [ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p]
        self.ch347dll.CH347SPI_Write.restype = ctypes.c_bool
        
        # Set the function argument types and return type for CH347SPI_Read
        self.ch347dll.CH347SPI_Read.argtypes = [ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.POINTER(ctypes.c_ulong), ctypes.c_void_p]
        self.ch347dll.CH347SPI_Read.restype = ctypes.c_bool
        
        # Set the function argument types and return type for CH347SPI_WriteRead
        self.ch347dll.CH347SPI_WriteRead.argtypes = [ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p]
        ...
        self.ch347dll.CH347StreamSPI4.argtypes = [ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p]
        ...
        self.ch347dll.CH347StreamI2C.argtypes = [ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p]
    ...
    @staticmethod
    def event_callback(self, iEventStatus):
        # Callback function implementation
        print("Callback event status:", iEventStatus)
        if iEventStatus == 0:
            # Device unplug event
    ...
    def set_device_notify(self, iDeviceID, iNotifyRoutine = event_callback):
        """
    ...
    def spi_change_cs(self, iStatus):
        """
    ...
    def spi_write_read(self, iChipSelect, iLength, ioBuffer):
        """
        ...
        Args:
            ...
            ioBuffer (ctypes.c_void_p): Points to a buffer that places the data to be written out from DOUT.
    ...
    def stream_spi4(self, iChipSelect, iLength, ioBuffer):
        ...
        Args:
            ...
            iLength (int): Number of bytes of data to be transferred.
            ioBuffer (ctypes.c_void_p): Points to a buffer that places data to be written out from DOUT.
    ...
    def CH347GPIO_Get(self) :
        ...
        CH347GPIO_Get = self.ch347dll.CH347GPIO_Get
        CH347GPIO_Get.argtypes = [ctypes.c_ulong, ctypes.POINTER(ctypes.c_ubyte), ctypes.POINTER(ctypes.c_ubyte)]
        CH347GPIO_Get.restype = ctypes.c_int  # 假设返回类型为int

        # 调用函数
        ilndex = ctypes.c_ulong(0)  # 示例参数
        iDir = ctypes.c_ubyte()  # 用于接收数据的变量
        iData = ctypes.c_ubyte()  # 用于接收数据的变量

        # 调用函数，注意使用byref来传递iDir和iData的引用
        result = CH347GPIO_Get(ilndex, ctypes.byref(iDir), ctypes.byref(iData))
        print(iDir)
        print(iData)
        return result
